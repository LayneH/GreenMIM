...
class MaskedAutoencoderSwin(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3,
                 ...
                 block_cls=Block, backbone_cls=SwinTransformer,
                 **kwargs):
        super().__init__()
        ...
        self.encoder = backbone_cls(img_size=img_size, patch_size=patch_size, in_chans=in_chans,
                 num_classes=0, embed_dim=embed_dim, depths=depths, num_heads=num_heads,
                 window_size=window_size, norm_layer=norm_layer, **kwargs)
...
def green_mim_swin_base_patch4_dec5121b1(**kwargs):
...
def green_mim_swin_base_patch4_win14_dec5121b1(**kwargs):
    model = MaskedAutoencoderSwin(
                # encoder configs
                img_size=224,
                patch_size=4,
                ...
                patch_norm=True,
                use_checkpoint=False,
                norm_layer=partial(nn.LayerNorm, eps=1e-6), # FIXME: 1e-6
                # decoder settings
                decoder_embed_dim=512,
                decoder_depth=1,
                decoder_num_heads=16,
...
def green_mim_swin_large_patch4_win14_dec5121b1(**kwargs):
    model = MaskedAutoencoderSwin(
                # encoder configs
                img_size=224,
                patch_size=4,
